...
ruta_archivos=r'C:\Users\sergi\OneDrive\Documents\Universidad\Informe_joven_investigador\Informe_final\Simulacion_global\Base'
secciones=ruta_archivos+'\\'+'SECCIONES.xlsx' #Archivo de secciones a entrenar
ruta_HEC_RAS_nivel=ruta_archivos+'\\'+'Niveles_ras_RC.xls' #Archivo de niveles (salida HEC-RAS)
ruta_HEC_RAS_caudal=ruta_archivos+'\\'+'Caudales_ras_RC.xls' #Archivo de caudal (salida HEC-RAS)
tributarios_HEC_HMS=ruta_archivos+'\\'+'Tributarios_RC_2018.xls' #Tributarios de HEC-HMS (salida HEC-HMS)
calidad=ruta_archivos+'\\'+'Calidad_Rio.xlsx' #Se llaman el archivo que contiene la conductividad y la temperatura de los afluentes (Gestión y archivos provenientes de dársenas y lago)
Estaciones=[10950, 10500, 10350, 9600, 9300,7050,6750, 6600, 6450, 6300,4350 ,4050, 3750, 2850, 150] #Estaciones dentro del río (Río Chicamocha)
cotas_menores=[2492.4,2489.55,2487.8,2487.8,2487.4,2485.2,2484.8,2485.2,2485.4,2485.2,2484.6,2484,2483.6,2484.31,2482.6] #Cotas menores río Chicamocha
junction=['J804','Q. EL TOTUMO A.D.','J786','J798','RIO CHICAMOCHA ALTO','LAGO SOCHAGOTA','PTAR',
          'Q. HONDA-LAGO SOCHA?','Q. SECA','ITP','Q. EL ORTIGAL','Q. TOIBITA A.D.','Q. CHORROBLANCO','RIO CHICAMOCHA ALTO 4'] #Aferentes rio chicamocha
salinidad_lago=pd.read_excel(ruta_archivos+'\Resultados\Lago\Salinidad_Lago.xls',header=6)
salinidad_ITP=pd.read_excel(ruta_archivos+'\Resultados\Darsenas\Darsena_2\Salinidad_D2.xls',header=6)
salinidad_lago=list(salinidad_lago[salinidad_lago.columns[1]])
salinidad_ITP=list(salinidad_ITP[salinidad_ITP.columns[1]])
...
#Se reemplazan las salinidades del Lago y el ITP
for i in range(0,len(salinidad_afl)):
    salinidad_afl[i][6]=salinidad_lago[i] #Lago
    salinidad_afl[i][11] = salinidad_ITP[i] #Dársena 2
# print(salinidad_afl)
...
pot_sal_gensa=potencial_de_salinidad(conc,0.0528)
pot_sal_siberia=potencial_de_salinidad(conc,0.088)
pot_sal_unidad_holanda=potencial_de_salinidad(conc,0.18)

# print(convertir_dataframe(nivel,fecha,Estaciones))
# print(convertir_dataframe(Caudales,fecha,Estaciones))
# print(convertir_dataframe(pot_sal,fecha,Estaciones))
...
convertir_dataframe(nivel,fecha,Estaciones).to_excel(ruta_guardado+'\\'+'Nivel.xlsx')
convertir_dataframe(Caudales,fecha,Estaciones).to_excel(ruta_guardado+'\\'+'Caudales.xlsx')
convertir_dataframe(conc,fecha,Estaciones).to_excel(ruta_guardado+'\\'+'salinidad.xlsx')
pd.DataFrame(convertir_dataframe(pot_sal_gensa,fecha,Estaciones)[7050]).to_excel(ruta_guardado+'\\'+'pot_sal_Gensa.xlsx')
pd.DataFrame(convertir_dataframe(pot_sal_siberia,fecha,Estaciones)[4350]).to_excel(ruta_guardado+'\\'+'pot_sal_Siberia.xlsx')
pd.DataFrame(convertir_dataframe(pot_sal_unidad_holanda,fecha,Estaciones)[150]).to_excel(ruta_guardado+'\\'+'pot_sal_Unidad_Holanda.xlsx')
