...
areas=lambda par1,par2,nivel:(par1*(nivel) + par2) # funcion anonima para calcular la velocidad mediante los parametros de caudal y area
...
def potencial_de_renovacion(concentraciones):
    ...
    for i in range(0,len(concentraciones)):
        a=(concentraciones[i][0]-concentraciones[i][1])/concentraciones[i][0]*100
        pr.append(a)
    return pr
...
###########################################################################################
# ruta=r'C:\Users\sergi\OneDrive\Documents\Universidad\Informe_joven_investigador\Informe_final\Simulacion_global\Base'
...
# Estaciones=[25319,22202,150] #Estaciones dentro del río (Quebrada Honda)
# Estaciones=[10950, 10500, 7050] #Estaciones dentro del río (Río Chicamocha)
# Estaciones=[7050, 6600, 4350] #Estaciones dentro del río (Río Chicamocha)
Estaciones=[4350, 3750, 150] #Estaciones dentro del río (Río Chicamocha)
...
convertir_dataframe(pot_rev,fecha,Estaciones[len(Estaciones)-1]).to_excel(ruta_guardado+'\\'+'pot_rev_Unidad_Holanda.xlsx')
plt.plot(pot_rev)
