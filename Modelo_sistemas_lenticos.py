...
def cargas_tranvase(caudales,conductividad,temperatura):
    sal=[]
    car = []
    for i in range(0,len(conductividad)):
        s = salinidad1(conductividad[i],temperatura[i])
        sal.append(s)
...
def salinizacion(vol_sis,cargas_transvase,cargas_afluentes,cargas_quebrada, caudales_efl,
                 con_inicial, tem_inicial, con_geo=2.5,tem_geo=21.2):
    """
    Parámetros
    ----------------------------------------------------------------------------------------------------------------------
    ...
    for i in range(0, 365):
        ...
        Mooas3=(Ci * vol_sis[i])+(cargas_afluentes[i]+cargas_transvase[i]+cargas_quebrada[i])*86400
...
def cambiar_formato(lista_variable,fechas):
    ...
    dss=dss.fillna('')
    return dss
def cargas_quebrada(caudal,salinidad):
    a=[caudal[i]*salinidad[i] for i in range(0,len(caudal))]
    return a
...
def graficar_potencial(x, y, niv_inf, niv_med, niv_sup, potencial='deficit hidrico',label='label'):
    ...
    values = dict(inf=-10000, niv_inf=niv_inf, niv_med=niv_med, niv_sup=niv_sup, sup=10000)
...
ruta_carpeta=r'C:\Users\sergi\OneDrive\Documents\Universidad\Informe_joven_investigador\Informe_final\Simulacion_global\Base\Lago_Sochagota'
...
ruta_carpeta=r'C:\Users\sergi\OneDrive\Documents\Universidad\Informe_joven_investigador\Informe_final\Simulacion_global\Base'
fec1_="01/01/2018"
fec2_="12/31/2018" #Modificar fecha según periodo simulado
vec_fecha=np.array(pd.date_range(fec1_,fec2_))
Quebrada_Honda=pd.read_excel(ruta_carpeta+'\\'+'Tributarios_QH_2018.xls',header=1)
...
salinidad_quebrada=pd.read_excel(ruta_carpeta+'\\'+'Salinidad_Quebrada_Honda.xlsx',header=0)
salinidad_quebrada=salinidad_quebrada[150]

#Varables climáticas
ruta_carpeta_clima=r'C:\Users\sergi\OneDrive\Documents\Universidad\Proyecto_BIO\Proyecto_Boyaca_BIO\Modelos\Modelo_sistemas_lenticos\Lago_Sochagota'
nombre_excel='Evaporacion'
Evaporacion=Abrir_excel(ruta_carpeta_clima,nombre_excel,'Sheet1')
nombre_excel='Precipitacion'
Precipitacion=Abrir_excel(ruta_carpeta_clima,nombre_excel,'Sheet1')
...
evap=Evaporacion
dq_ant=dq_ant
q_trans=[0 for i in range(0,len(fecha))]
aper_com=Regimen_compuerta['Porcentaje de apertura (%)']
q_hid=Quebrada_Honda['Quebrada_Honda']
#Altura y caudales de salida del Lago
Alturas_lago,Caudal_salida_lago=Dinamica_lago(No,Cfl,Cfc,prec,evap,dq_ant,aper_com,q_hid,q_trans)
...
# graficar(Alturas_cal,fecha,ylabel='Alturas',label='Simulated data')
# # graficar(observados[observados.columns[0]],fecha,ylabel='Alturas',label='Observed data',
#          color='red')
# plt.show()
...
cargas_QH=cargas_quebrada(list(Quebrada_Honda[Quebrada_Honda.columns[0]]),list(salinidad_quebrada))

con_inicial=10.16
tem_inicial=20

con_trans=[0 for i in range(0,len(fecha))]
tem_trans=[0 for i in range(0,len(fecha))]
car_tranvase=cargas_tranvase(q_trans,con_trans,tem_trans)

#Salinidad de salida del Lago Sochagota en el ciclo de gestión
salinidad_salida_lago=salinizacion(vol_sis, cargas_afluentes,cargas_QH, car_tranvase,caudales_efl, con_inicial, tem_inicial,con_geo=2.5,tem_geo=21.2)

#Se convierte a DSS
DSS_sal=cambiar_formato(salinidad_salida_lago,fecha)
DSS_cau=cambiar_formato(list(caudales_efl),fecha)

#Se exportan los resultados a Excel
DSS_sal.to_excel(ruta_carpeta+'\Resultados\Lago\Salinidad_Lago.xls',index=False,header=False)
DSS_cau.to_excel(ruta_carpeta+'\Resultados\Lago\Efluentes_lago.xls',index=False,header=False)

#Calculo de los potenciales
pot_rev=potencial_de_renovacion(vol_sis,caudales_efl)
pot_def_hid=potencial_de_deficit(Alturas_lago,2.273)
pot_sal=potencial_de_salinizacion(salinidad_salida_lago,0.1287,17.15)
...
print("Déficit hídrico:" + str(conceptoGlobalPWd))
...
print("Potencial de renovación:" + str(conceptoGlobalPR))
...
print("Potencial de salinización:" + str(conceptoGlobalPS))
...
print("Concepto Global OSI:"+ str(conceptoGlobalOSI))
...
plt.savefig(ruta_carpeta+'\Resultados\Lago\climatologia.png')
plt.show()
graficar_potencial(fecha, pot_rev, -1.4, -0.39, 5.79, potencial='renovación',label='Renewal potential')
plt.savefig(ruta_carpeta+'\Resultados\Lago\Potencial_de_renovacion_2018_esc_base.png')
plt.show()
graficar_potencial(fecha, pot_sal, -7.41, -69.14, -72.22, potencial='salinizacion',label='Salinization potential')
plt.savefig(ruta_carpeta+'\Resultados\Lago\Potencial_de_salinizacion_2018_esc_base.png')
plt.show()
graficar_potencial(fecha, pot_def_hid, -0.69, 0.16, 4.43, potencial='deficit hidrico',label='Water deficit potential')
plt.savefig(ruta_carpeta+'\Resultados\Lago\Potencial_de_deficit_2018_esc_base.png')
plt.show()
grafica_OSI(fecha,serieOSI)
plt.savefig(ruta_carpeta+'\Resultados\Lago\OSI_2018_esc_base.png')
plt.show()
pie_chart(PorcentajeMuyAltoOSI, PorcentajeAltoOSI, PorcentajeBajoOSI, PorcentajeModeradoOSI)
plt.savefig(ruta_carpeta+'\Resultados\Lago\Pie_chart_2018_esc_base.png')
plt.show()

Alturas_lago=[Alturas_lago[i]+Cfc for i in range(0,len(Alturas_lago))]

grafica_niveles_caudales(Caudal_salida_lago,Alturas_lago,fecha,'Efluent from Lago Sochagota','Lago Sochagota')
# graficar(Alturas_cal,fecha,'Water level [m]',label='Lago Sochagota')
plt.savefig(ruta_carpeta+'\Resultados\Lago\Lago_Niveles.png')
plt.show()
# graficar(Caudal_salida_lago,fecha,'Flow [m3/s]',label='Efluent from Lago Sochagota [m]')
# plt.savefig(ruta_carpeta+'\Resultados\Lago\Caudales.png')
# plt.show()
